...
export default function UsuarioCadastrar() {
  ...
  useEffect(() =>{
    async function getUsuario(){
      var response = await api.get('/cliente/'+idUsuario);

      setNome(response.data.USR_NOME);
      setEmail(response.data.USR_EMAIL);
      setSenha(response.data.USR_SENHA);
      setTipo(response.data.USR_TIPO);
    }
  ...
  async function handleSubmit(){
    ...
    if(nome != '' && email != '' && senha != '' && tipo != ''){
      const response = await api.post('/cliente/update',data);

      if(response.status === 201){
        window.location.href="/admin/usuarios";
      }else{
        alert('Erro ao atualiazar o usuário');
      }
    }else{
